...
def getBGRFromFrame(frame, width, height):
    frame_array = np.array(frame)
    oldimg = Image.fromarray(frame_array, 'RGB')

    # frame_bytes = io.BytesIO()
    # frame_bytes.write(frame)
    # oldimg = Image.frombytes('RGB', (width, height), bytes(frame))
    # oldimg.show()
    # oldimg = Image.fromstring(frame.tostring())
    # oldimg = Image.open(frame_bytes)
    #  # BMP 4 byte align
    newWidth = width & 0xFFFFFFFC
    newHeight = height & 0xFFFFFFFE
    ...
    img.transpose(Image.FLIP_TOP_BOTTOM).convert('RGB').save(BMP_bytes, format='BMP')
    bgr_buffer = bytes(BMP_bytes.getvalue()[54:])
    # bgr_buffer = bytes(frame)

    return BufferInfo(newWidth, newHeight, bgr_buffer)

def getI420FromFrame(frame, width, height):
    oldimg = frame
